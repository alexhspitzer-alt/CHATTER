...
const SUPPORTED_HOT_WORDS = ['culvert', 'latch', 'threshold', 'cinder', 'spigot', 'acorn', 'cobblestone', 'wick', 'awning'];
...
async function loadOptionalPack(word, path) {
  try {
    return await loadJson(path);
  } catch (_error) {
    addAudit(`OPTIONAL PACK MISSING: ${word} -> fallback to base chatter`);
    return null;
  }
}
...
async function bootstrap() {
  const [
    ...
    loadOptionalPack('acorn', OPTIONAL_PACK_PATHS.acorn),
    loadOptionalPack('cobblestone', OPTIONAL_PACK_PATHS.cobblestone),
    loadOptionalPack('wick', OPTIONAL_PACK_PATHS.wick),
    loadOptionalPack('awning', OPTIONAL_PACK_PATHS.awning),
  ]);
  ...
  state.chatterPacks = {
    ...
    cinder: cinderPack,
    spigot: spigotPack,
    acorn: acornPack ?? baseData.all,
    cobblestone: cobblestonePack ?? baseData.all,
    wick: wickPack ?? baseData.all,
    awning: awningPack ?? baseData.all,
  };
